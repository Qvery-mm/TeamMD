<?php
	$mysqli = new mysqli("127.0.0.1","root","","ecolab");
						for ($i=4;$i<=7;$i++)
								{
								$sql = "SELECT * FROM `ecolab_events` WHERE `id` = '$i'";
								$result = $mysqli->query($sql);
								$res = $result->fetch_assoc();
								if(count($res)>0)

										$ev_title[$i] = $res["title"];
										$ev_lider[$i] = $res["lider"];
										$ev_place[$i] = $res["place"];	
										$ev_src[$i] = $res["images"];
										$ev_date[$i]=$res["date"];
							 }
							 $mysqli->close();
...
<html lang="ru">
   ...
   <meta http-equiv="x-ua-compatible" content="ie=edge">
	...
	<header id="fh5co-header" class="fh5co-cover" role="banner" style="background: #0dc70a; height: 420px;">
				...
				<div class="col-md-8 col-md-offset-2 text-center">
					<div class="display-t">
						<div class="display-tc animate-box" data-animate-effect="fadeIn">
							...
							<h2>Прими участие, спаси природу</h2>
							<div class="row">
									<div class="col-md-8 col-md-offset-2">
										<div class="form-group">
											<a class="btn btn-default">Присоединиться</a>
										</div>
									</div>
							</div>
						</div>
	...
	<div id="fh5co-project" class="event_fh5co">
		<div class="container">
			...
			<div class="col-half">
				...
				<div class="project-grid animate-box">
					...
					<div class="desc">
						<h3><?=$ev_title[7]?></h3>
						<p>Приглашаем вас принять участие в одноЙ из наших акций.</p>
						<p><?=$ev_place[7].", ".$ev_date[5]?></p>
						<a href="#" class="btn-post take_a_part">Участвовать</a>
					</div>
				</div>
			</div>
			</div>
			<div class="project-content event_content"></div>
		</div>
		<form class="bottom_btn_post chat-form" method="post">
					<button type="submit" class="btn-post bottom">Ещё события</button>
			</form>
	</div>
